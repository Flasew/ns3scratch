...
void
RxBufTrace (const uint32_t i, const uint32_t j)
{
  std::cout << "Rx Next expected seq number: " << j << std::endl;
}

void
TxBufTrace (const uint32_t i, const uint32_t j)
{
  std::cout << "Tx Next unacked seq number: " << j << std::endl;
}
...
int main(int argc, char * argv[]) {
  ...
  LogComponentEnable("TcpTxBuffer", LOG_LOGIC);
  ...
  tch.Install(TopoHelper::GetLink(0, 1).Get(0));
  tch.Install(TopoHelper::GetLink(0, 1).Get(1));

  Ipv4AddressHelper address;
  address.SetBase("10.0.0.0", "255.255.255.0");
  TopoHelper::AssignIP(address, 0, 1);
  ...
  BulkSendHelper source ("ns3::TcpSocketFactory",
      InetSocketAddress (TopoHelper::GetIf(0, 1).GetAddress(1), port));
  // Set the amount of data to send in bytes.  Zero is unlimited.
  source.SetAttribute ("MaxBytes", UintegerValue(max_bytes));
  // source.SetAttribute("PacketSize", UintegerValue(1500));
  ApplicationContainer sourceApps = source.Install(TopoHelper::allNodes.Get(0));
  //Ptr<Socket> socket = /*DynamicCast<TcpSocket>(*/DynamicCast<BulkSendApplication>(sourceApps.Get(0))->GetSocket()/*)*/;
  //socket->SetAttribute("SndBufSize", ns3::UintegerValue(262144));;
  ...
  Config::Connect(
      "/NodeList/*/$ns3::TrafficControlLayer/RootQueueDiscList/*/BytesInQueue",
      MakeCallback (&QlTrace));
  Config::Connect ("/NodeList/*/ApplicationList/*/$ns3::PacketSink/Rx",
      MakeCallback (&RxTrace));
  Config::ConnectWithoutContext (
      "/NodeList/1/$ns3::TrafficControlLayer/RootQueueDiscList/0/Drop",
      MakeCallback (&DropTrace));
  Config::ConnectWithoutContext (
      "/NodeList/*/$ns3::TcpL4Protocol/SocketList/*/Rx",
      MakeCallback (&RxBufTrace));
  Config::ConnectWithoutContext (
      "/NodeList/0/$ns3::TcpL4Protocol/SocketList/0/Tx",
      MakeCallback (&TxBufTrace));
  Config::ConnectWithoutContext (
      "/NodeList/0/$ns3::TcpL4Protocol/SocketList/1/Tx",
      MakeCallback (&TxBufTrace));
  // Config::ConnectWithoutContext (
