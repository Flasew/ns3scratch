...
int main(int argc, char * argv[]) {
  ...
  //Config::SetDefault("ns3::TcpSocket::SegmentSize", UintegerValue (524288));
  ...
  */
  Address sinkAddress(InetSocketAddress(TopoHelper::GetIf(1, 3).GetAddress(0), port));
  Ptr<Socket> ns3TcpSocket = Socket::CreateSocket (TopoHelper::allNodes.Get(0), TcpSocketFactory::GetTypeId());
  ns3TcpSocket->TraceConnectWithoutContext ("CongestionWindow", MakeCallback (&CwndTrace));

  Ptr<BulkSendApplication> app = CreateObject<BulkSendApplication> ();
  app->SetUp (ns3TcpSocket, sinkAddress, max_bytes);
  ...
  //(DynamicCast<BulkSendApplication> (sinkApps.Get(0)))->MakeSocket();
  std::cout << "passed make socket" << std::endl;
