...
void DropTrace (const Ptr<QueueDiscItem const> qip)
{
  dropped++;
  // qip->Print(std::cout);
  if (drlog.is_open())
...
int main(int argc, char * argv[]) {
  ...
  frdata = std::vector<uint64_t>(nflows);
  lodata = std::vector<uint64_t>(nflows);
  syndata = std::vector<uint64_t>(nflows);
  estdata = std::vector<uint64_t>(nflows);
  ...
  Address sinkAddress(InetSocketAddress(TopoHelper::GetIf(1, 3).GetAddress(0), portInit));
  std::vector< Ptr<BulkSendApplication> > sendApps;
  ...
  for (uint64_t i = 0; i < nflows; i++) {
    ...
    app->SetUp(ns3TcpSocket, sinkAddress, max_bytes/nflows);
